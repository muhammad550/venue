...
class Property extends Bookable
{
    ...
    public $item_loop = 'Property::frontend.layouts.search.loop.loop-gird';
    public $item_loop_list = 'Property::frontend.layouts.search.loop.loop-list';
    ...
    {
        $model_property = parent::query()->select("bc_properties.*");
        $model_property->where("bc_properties.status", "publish");
        if(!empty($agent_id  = $request->query('agent_id'))){
            $model_property->where('create_user',$agent_id);
        }
        if (!empty($category_id = $request->query('category_id'))) {
            $model_property->join('bc_property_category_relationships as pcs', 'pcs.property_id', "bc_properties.id")->where('pcs.category_id', $category_id);
        }
        ...
        if (!empty($price_range = $request->query('amount'))) {
            $price_range = explode(',', $price_range);
            $pri_from = (int)$price_range[0];
            $pri_to = isset($price_range[1]) ? (int)$price_range[1] : 0;
            if($pri_to > $pri_from && $pri_from > 0) {
                $raw_sql_min_max = "( (IFNULL(bc_properties.price_from,0) > 0 and bc_properties.price_from >= ? ) OR (IFNULL(bc_properties.price_from,0) <= 0 and bc_properties.price >= ? ) )
                                AND ( (IFNULL(bc_properties.price_from,0) > 0 and bc_properties.price_from <= ? ) OR (IFNULL(bc_properties.price_from,0) <= 0 and bc_properties.price <= ? ) )";
        ...
        if (is_array($terms) && !empty($terms)) {
            $model_property->join('bc_property_term as tt', 'tt.target_id', "bc_properties.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if(!empty($property_type = $request->query("property_type"))) {
            $model_property->where("bc_properties.property_type",$property_type);
        }
        if(!empty($garage = $request->query("garage"))) {
            $model_property->where("bc_properties.garages",$garage);
        }
        if(!empty($bathroom = $request->query("bathroom")) && $bathroom > 0) {
            $model_property->where("bc_properties.bathroom",$bathroom);
        }
        if(!empty($bedroom = $request->query("bedroom")) && $bedroom > 0) {
            $model_property->where("bc_properties.bed",$bedroom);
        }
        if(!empty($year_built = $request->query("year_built")) && $year_built > 0) {
            // dd($year_built);
            $model_property->where("bc_properties.year_built",$year_built);
        }
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_property->leftJoin('bc_property_translations', function ($join) {
                    $join->on('bc_properties.id', '=', 'bc_property_translations.origin_id');
                });
                $model_property->where('bc_property_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_property->where('bc_properties.title', 'LIKE', '%' . $service_name . '%');
            }
        }

        if(!empty($price_range_type = $request->query('price_range'))){
            $model_property->where('bc_properties.price_range','>=',$price_range_type);
        }
        ...
        return $model_property->with(['location','hasWishList','translations','user','categories'])->paginate($limit);
    }
    ...
    public function Category(){
        return $this->belongsTo('Modules\Property\Models\PropertyCategory','category_id','id');
    }
    ...
    public function getDisplayPriceSingleAttribute()
    {
        $price_html = '';
        $price_html .= (!empty($this->price_from) and $this->price_from > 0) ? format_money($this->price_from) : 0;
        if(!empty($this->price) and $this->price > 0){
    ...
    public function categories(){
        return $this->belongsToMany(PropertyCategory::class, 'bc_property_category_relationships', 'property_id', 'category_id');
    }

    public function getIsOpenAttribute(){
        $current_day = date('N');
        $current_house = date('H:i');
        if(!empty($this->open_hours[$current_day]) &&  $current_house >= $this->open_hours[$current_day]['from'] && $current_house < $this->open_hours[$current_day]['to']){
            return true;
        }
       return false;
    }

    public function getListProperties($category_id = false, $location = false){
        $model_property = parent::query()->select("bc_properties.*");
        $model_property->where("bc_properties.status", "publish");
