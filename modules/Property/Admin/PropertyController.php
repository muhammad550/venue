...
class PropertyController extends AdminController
{
    ...
    public function showContact(Request $request) {
        ...
        if (count($rows) > 0) {
            foreach($rows as $row) {
                $property = $this->property::where('id', $row->object_id)->first();
                if ($property) {
                    $row->nameProperty = $property->title;
                } else {
                    $row->nameProperty = 'Default Title'; // Or handle the null case as needed
                }
        
                $vendor = DB::table('users')->select(DB::raw('CONCAT(first_name, " ", last_name) AS name'))->where('id', $row->vendor_id)->first();
                if ($vendor) {
                    $row->nameVendor = $vendor->name;
                } else {
                    $row->nameVendor = 'Unknown Vendor'; // Or handle the null case as needed
                }
            }
        }
